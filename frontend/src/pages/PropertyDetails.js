...
const PropertyDetails = () => {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:5000/api/inmuebles/${id}`)
      .then((res) => {
        setProperty(res.data.info[0]);
      })
      .catch((err) => {
        console.log(err);
      });
      ...
      {property ? (
        <div className="container mx-auto min-h-[800px] mb-14">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
            ...
            {!property.priceSecond === 'nan' ? (
              <div className="text-3xl font-semibold text-violet-600">
                <p>
                  Rent: ${property.price} - Sale: ${property.priceSecond}
                </p>
                <p></p>
              </div>
            ) : (
              <div className="text-3xl font-semibold text-violet-600">
                <p>
                  {property.business}: ${property.price}
                </p>
              </div>
            )}
          </div>
